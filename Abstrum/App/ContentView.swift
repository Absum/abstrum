...
struct ContentView: View {
    ...
    init() {
        #if DEBUG
        switch ProcessInfo.processInfo.environment["ABSTRUM_TAB"] {
        ...
        // suppress it whenever a ABSTRUM_ navigation flag is set, so QA flows work.
        var show = !UserDefaults.standard.bool(forKey: "didOnboarding")
        #if DEBUG
        let env = ProcessInfo.processInfo.environment
        if env["ABSTRUM_ONBOARDING"] != nil { show = true }
        else if env.keys.contains(where: { $0.hasPrefix("ABSTRUM_") }) { show = false }
        #endif
