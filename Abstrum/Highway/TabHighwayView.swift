...
struct TabHighwayView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            #if DEBUG
            if let id = ProcessInfo.processInfo.environment["ABSTRUM_HIGHWAY"],
               let match = HighwayLibrary.all.first(where: { $0.id == id }) {
                track = match
            }
            if ProcessInfo.processInfo.environment["ABSTRUM_IMPORT"] != nil {
                showImport = true
            }
            if ProcessInfo.processInfo.environment["ABSTRUM_SEED_IMPORT"] != nil, imports.songs.isEmpty {
                imports.add(title: "My Riff", bpm: 100,
                            steps: [(5, 0, 0.5), (5, 1, 0.5), (5, 3, 1), (4, 3, 1), (4, 1, 2)])
            }
            if ProcessInfo.processInfo.environment["ABSTRUM_EDIT"] != nil {
                editingSong = imports.songs.first
            }
            if ProcessInfo.processInfo.environment["ABSTRUM_PLAY_IMPORT"] != nil {
